...
Array.prototype.flatten = function() {
	...
	for (var i = 0; i < this.length; i++) {
		var item = this[i];
		if (item.flatten) array = array.concat(item.flatten());
...
function random(a, b) {
	a + this[Math.floor(Math.random()*(b - a + 1))];
}
...
function range(start, end) {
	...
	var inc = (start < end);
	var d = (inc ? 1 : -1);
	for (var i = start; (inc && i <= end) || (!inc && i >= end); i += d) {
		array.push(i);
	}
	return array;
}
...
function toCssValue(width, height) {
